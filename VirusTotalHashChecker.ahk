...
guiShow() {
	...
	If ExistInstallDir {
		...
		If extendContextKey
			Control Check,,, ahk_id %RadioIfShiftId%
	}
	...
	ChangeRadioDisplayItemOpt:
		If ExistInstallDir {
			...
			RegRead extendContextKey, HKEY_CLASSES_ROOT, *\shell\VirusTotal hash check\command
			If ((extendContextKey && !RadioIfShift) || (Not extendContextKey && RadioIfShift))
			{
...
Hower(wParam, lParam, msg, hwnd) {
	...
	hCurs:=DllCall("LoadCursor","UInt",0,"Int",IDC_HAND := 32649,"UInt")
